...
#analysing/reading each and every page in the pdfs and extracting text from them
...
#It will be very large in size for Gemini to handle so we are dividing into chunks
def get_text_chunks(text):
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=10000, chunk_overlap=1000) #chunk overlap is for better understanding by AI Model
    chunks = text_splitter.split_text(text)
    return chunks

#Converting texts into vectors, machine understandable using FAISS
...
#Letting Gemini give response with the prompt below, so that it give accurate answers
