...
docvectorpine.init(api_key=os.getenv("PINECONE_API_KEY"), environment="us-west1-gcp")   
# Initialize Pinecone index
index_name = "pdf-index"
if index_name not in docvectorpine.list_indexes():
    docvectorpine.create_index(index_name, dimension=1536)   
...
def get_vector_store(text_chunks):
    ...
    # Pinecone doesn't require explicit saving like FAISS
...
def user_input(user_question):
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    # Load vectors from Pinecone
    new_db = Pinecone(index_name=index_name, embedding=embeddings)
